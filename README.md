...
4. **Database migrations (MySQL)**  
   - On each deploy, **`scripts/start-standalone.cjs` runs `migrate.sql` automatically** when any DB env var is present (unless `SKIP_DB_MIGRATE=1`). Uses `CREATE TABLE IF NOT EXISTS`, so it is safe to repeat. If MySQL is unreachable (**`ETIMEDOUT`**), the app **still starts** so Railway healthchecks pass — fix networking (link MySQL, use private `DATABASE_URL`), then redeploy or run SQL manually. Set **`MIGRATE_EXIT_ON_FAIL=1`** only if you want the container to **exit** when migrate fails.  
